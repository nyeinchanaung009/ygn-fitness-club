...
@section('content')
    <div class="w-[95%] mx-auto">
        <div class="border border-gray-900 py-3 px-5 rounded bg-black flex">
            ...
            <h2 class="flex-1 text-center font-semibold text-white"><span class="text-slate-400"><i class="bi bi-buildings me-3 text-lg"></i></span>Create new Plan</h2>
        </div>

        <div class="sm:w-10/12 md:w-8/12 lg:w-6/12 mx-auto  border-slate-800 rounded p-3 mt-5 pb-6">
            
            <form action="{{route('plan.store')}}" class="max-w-sm mx-auto" method="POST">
                @csrf
                @if(session('error'))
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" class="bg-gray-100 text-gray-900 text-sm rounded blcok w-full p-2.5 focus:ring-2 focus:ring-orange-500" placeholder="plan name">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="price" name="price" value="{{ old('price') }}" class="bg-gray-100 text-gray-900 text-sm rounded blcok w-full p-2.5 focus:ring-2 focus:ring-orange-500" placeholder="price">
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-5 flex justify-between items-center w-full">
                    <div class="w-full">
                        ...
                        <input type="number" id="period" name="period" value="{{ old('period') }}" class="bg-gray-100 text-gray-900 text-sm rounded w-[98%] mx-auto p-2.5 focus:ring-2 focus:ring-orange-500" placeholder="period">
                        @error('period')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
