...
@section('content')
    <div class="w-[95%] mx-auto">
        <div class="border border-gray-900 py-3 px-5 rounded bg-black flex">
            ...
            <h2 class="flex-1 text-center font-semibold text-white"><span class="text-slate-400"><i class="bi bi-buildings me-3 text-lg"></i></span>Update Branch</h2>
        </div>

        <div class="sm:w-10/12 md:w-8/12 lg:w-6/12 mx-auto  border-slate-800 rounded p-3 mt-5 pb-6">
            
            <form action="{{route('branch.update',$branch->id)}}" class="max-w-sm mx-auto" method="POST">
                @csrf @method('PUT')
                @if(session('error'))
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="name" name="name" value="{{ $branch->name }}" class="bg-gray-100 text-gray-900 text-sm rounded blcok w-full p-2.5 focus:ring-2 focus:ring-orange-500" placeholder="branch name">
                    @error('name')
                ...
                <div class="mb-5">
                    ...
                    <textarea name="address" id="address" class="w-full rounded block bg-gray-100 text-gray-900 text-sm mb-2 font-medium focus:ring-2 focus:ring-orange-500 p-2.5" placeholder="Address" rows="7">
                        {{ $branch->address }}
                    </textarea>
                    @error('address')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
